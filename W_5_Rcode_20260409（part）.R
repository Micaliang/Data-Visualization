...
# Q3(这个回去研究一下)
d1<-subset(diamonds,
           subset=(cart<1&price<=5000&clarity=="IF"))

ggplot(d1,aes(x=carat,y=price,color=color))+
  geom_line()

ggplot(d1,aes(x=carat,y=price,color=color))+
  geom_area(aes(fill=cut,alpha=0.6)
            
# Scatter plot
library(ggplot2)
ggplot(trees,aes(x=Girth,y=Height))+geom_point()

# defualt shape is 16,size=2
ggplot(trees,aes(x=Girth,y=Height))+geom_point(shape=21,size=5)

# group based on 3rd variable
# if the 3rd variable is discrete
ggplot(mpg,aes(x=cty,y=hwy,shape=class))+geom_point()
ggplot(mpg,aes(x=cty,y=hwy,color=class))+geom_point()
ggplot(mpg,aes(x=cty,y=hwy,color=class,shape=class))+geom_point()
...
ggplot(mtcars,aes(x=drat,y=wt,fill=qsec))+
  geom_point(shape=21,color="blue",fill="blue",size=4,alpha=0.5)
...
ggplot(Loblolly,aes(x=age,y=height))+geom_point(position = position_jitter())
# default 40% of data resolve
...
ggplot(trees,aes(x=Girth,y=Height))+geom_point()+
  geom_smooth(method = lm,level=0.99) # 99% confidence interval
...
ggplot(trees,aes(x=Girth,y=Height))+geom_point()+
  geom_smooth(method = loess) # locally weighted polynomial curve
...
ggplot(new,aes(x=drat,y=wt))+geom_point()+
  geom_text(aes(label=CARS),size = 3,vjust=1,hjust=0.5)
...
ggplot(birthwt,aes(x=factor(race),y=bwt))+
  geom_boxplot(outlier.size=3,color = "red",outlier.fill = "pink")
